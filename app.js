...
function handleItemsPerPageChange() {
  ...
  const newValue = parseInt(elements.itemsPerPage.value, 10);
  ...
  try {
    if (performance.memory) {
      ...
      if (limitMB && totalMB > limitMB) {
        elements.memoryInfo.innerHTML += ` <span class="text-muted">(limit ${limitMB}MB)</span>`;
      }
    } else {
...
function getTotalPages() {
  return Math.ceil(state.pdfs.length / state.itemsPerPage) || 1;
}
...
function updatePaginationInfo() {
  ...
  const startIndex = totalItems ? (state.currentPage - 1) * state.itemsPerPage + 1 : 0;
  const endIndex = totalItems ? Math.min(state.currentPage * state.itemsPerPage, totalItems) : 0;
...
function renderPaginationControls() {
  ...
  elements.paginationList.appendChild(
    createPageItem(state.currentPage - 1, "Previous", false, state.currentPage === 1)
  );
  ...
  elements.paginationList.appendChild(
    createPageItem(state.currentPage + 1, "Next", false, state.currentPage === totalPages)
  );
}
